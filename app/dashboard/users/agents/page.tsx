...
const AgentsPage = () => {
  ...
  const filteredAgents = useMemo(() => {
    return allUsers.filter((user) => {
      // Filter for 'agent' role for the Agents sub-tab
    ...
    <div>
      ...
      <div className="flex bg-white p-6 shadow rounded-md flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Dashboard / Users / Agents</p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="mb-8">
          <UserExports setShowFilters={setShowFilters} filteredUsers={filteredAgents} />
        ...
        <GeneralPagination datas={filteredAgents} setPaginatedData={setPaginatedUsers} />

        {filteredAgents.length === 0 && (
          <div className="text-center py-8 text-gray-500">No agent found matching your criteria</div>
        )}
      </div>
    </div>
  )
