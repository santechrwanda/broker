...
const UsersPage = () => {
  ...
  const filteredCustomers = useMemo(() => {
    return allUsers.filter((customer) => {
      // Filter for 'client' role for the Customers sub-tab
      const matchesRole = customer.role === "client"
      ...
      const matchesDate = !selectedDate || customer.role.includes(selectedDate) // This seems to be filtering by role, not date. Keep as is if intended.
    ...
    <div>
      ...
      <div className="flex bg-white p-6 shadow rounded-md flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Dashboard / Users / Customers</p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="mb-8">
          <UserExports setShowFilters={setShowFilters} filteredUsers={filteredCustomers} />
        ...
        <GeneralPagination datas={filteredCustomers} setPaginatedData={setPaginatedUsers} />

        {filteredCustomers.length === 0 && (
          <div className="text-center py-8 text-gray-500">No customer found matching your criteria</div>
        )}
      </div>
    </div>
  )
